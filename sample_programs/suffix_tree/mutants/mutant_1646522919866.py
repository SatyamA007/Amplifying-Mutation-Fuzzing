...
class SuffixTree(object):
    ...
    def _split_edge(self, edge, suffix):
        self.nodes.append(Node())
        e = Edge(edge.first_char_index, edge.first_char_index + suffix.length, suffix.source_node_index, len(self.nodes) - 2)
...
def suffix_tree(string:str):
    s = SuffixTree(string, case_insensitive=True)
    startIdx = randrange(0, len(string))
    endIdx = randrange(0, len(string)+1)
    if startIdx>endIdx:
        t = startIdx
        startIdx = endIdx
        endIdx = t
    substr = string[startIdx:endIdx]
    return s.find_substring(substr)
    

#print(suffix_tree("dogHasaPotato"))        